...
NUM_COLS = 3
(COL_LABEL, COL_URI, COL_CAN_MIGRATE) = range(NUM_COLS)
...
class vmmMigrateDialog(vmmGObjectUI):
    ...
    def show_instance(cls, parentobj, vm):
        try:
            ...
            parentobj.err.show_err(_("Error launching migrate dialog: %s") % str(e))
    ...
    def _destconn_changed(self, src):
        ...
        if can_migrate and uri in self._connobjs:
            ...
            if not destconn.is_remote():
                tunnel_warning = _(
                    "A remotely accessible libvirt URI "
                ...
                tunnel_warning = "<span size='small'>%s</span>" % tunnel_warning
        ...
        uiutil.set_grid_row_visible(self.widget("migrate-tunnel-warning-box"), bool(tunnel_warning))
    ...
    def _populate_destconn(self):
        ...
        if not any([row[COL_CAN_MIGRATE] for row in rows]):
            rows.insert(0, [_("No usable connections available."), None, False])
    ...
    def _finish(self):
        try:
            ...
            self.err.show_err((_("Uncaught error validating input: %s") % str(e)), details=details)
            return
    ...
    def _async_migrate(
        self, asyncjob, origvm, origdconn, migrate_uri, tunnel, unsafe, temporary, xml
    ):
        ...
        log.debug(
            "Migrating vm=%s from %s to %s", vm.get_name(), srcconn.get_uri(), dstconn.get_uri()
        )

        vm.migrate(dstconn, migrate_uri, tunnel, unsafe, temporary, xml, meter=meter)
